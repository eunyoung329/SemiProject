...
function render(itemList, filteredItems) {
  ...
  let content = "";
  infowindow = new kakao.maps.InfoWindow({ // let이나 const 키워드로 선언된 infowindow 변수에 할당
    content: content
  })
  ...
  $.ajax({
    ...
    success: function (maker) {
      ...
      for (var i = 0; i < maker.length; i++) {
        ...
        let id = maker[i].rest_id;
        var img = maker[i].rest_img;
        // 클로저를 활용하여 마커 보존
          ...
          kakao.maps.event.addListener(marker, 'click', function () {
            ...
            $.ajax({
              ...
              success: function (pick) {
                ...
                console.log(pick + "000000000000000000000");
                if (pick != null) {
  ...
  for (let item of itemsToRender) {
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    var infowindow = null; // infowindow 변수를 블록 스코프로 선언

    document.getElementById(item.id).addEventListener("dblclick", function () {
      var latlng = new kakao.maps.LatLng(item.lng, item.lat);
      var marker = new kakao.maps.Marker({
        map: map,
        position: latlng
      })
