...
public class SendEmailServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String subject = "[Muctopia 프로젝트] 회원 가입 이메일 인증번호"; // 제목
		
		String fromEmail = "kks4116@gmail.com"; // 보내는 사람으로 표시될 이메일 (이메일 따라서 안될수도 있음)
		String fromUsername = "[Muktopia_SignUp_Center]"; // 보내는 사람 이름
		...
		final String password = "xhfecssfyztlcpbl"; // 발급 받은 비밀번호
		// 메일 옵션 설정
		Properties props = new Properties();
		
		// 중요
		props.put("mail.transport.protocol", "smtp");//프로토콜
		props.put("mail.smtp.host", "smtp.gmail.com");//호스트
		props.put("mail.smtp.port", "587"); //465, 587//포트
		props.put("mail.smtp.auth", "true");//javax.mail 라이브러리에서 사용하는 요소.
		// 추가 옵션
		...
		try {
			...
			sb.append("<h3>[Muctopia 프로젝트] 회원 가입 인증 번호입니다.</h3>\n");
